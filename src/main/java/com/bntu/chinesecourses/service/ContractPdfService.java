...
public class ContractPdfService {
  ...
  private byte[] buildWord(
    ...
    try (XWPFDocument document = new XWPFDocument(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      String birthDate = person.getBirthDate() == null ? "____________" : person.getBirthDate().format(DateTimeFormatter.ofPattern("dd.MM.yyyy"));
      String phone = safe(person.getPhone());
      String priceValue = finalPrice.setScale(2, RoundingMode.HALF_UP).toPlainString();

      addCenteredBold(document, "ДОГОВОР №________", 14);
      ...
      addLeft(document, "г. Минск                                                                                                                                                      «___»___________2025 г.", 11);
      addSpacer(document, 1);

      addJustified(document,
          "Белорусский национальный технический университет (далее – БНТУ) в лице первого проректора Сафонова Андрея Ивановича, "
              + "действующего на основании доверенности №01-27 / 178 от 11.01.2024, с одной стороны, и граждане ",
          11, false);
      addSpacer(document, 1);
      ...
      addJustified(document,
          "1.\tОсвоение Слушателем образовательной программы дополнительного образования «Китайский язык для детей и подростков» "
              + "обучающих курсов по изучению китайского языка на платной основе в объёме 64 учебных часа.",
          11, false);
      addJustified(document, "2.\tСрок обучения: с  15.09.2025      по        12.01.2026.", 11, false);
      addJustified(document,
          "Стоимость обучения на момент заключения настоящего договора составляет " + priceValue
      ...
      addJustified(document,
          "3.\tСтоимость обучения, предусмотренная настоящим договором, может изменяться в случаях увеличения тарифной ставки "
              + "первого разряда для оплаты труда работников бюджетных организаций, изменения условий оплаты труда, рост тарифов "
              + "на коммунальные услуги и иные ценообразующие факторы, применяемые при формировании данной услуги. Цена может "
              + "изменяться также в случае индексации доходов населения в связи с опубликованием индекса потребительских цен в "
      ...
      addJustified(document,
          "4.\tОплата за обучение на основании настоящего договора осуществляется Слушателем на текущий (расчётный) счёт "
              + "по учёту внебюджетных средств БНТУ.",
          11, false);
      addSpacer(document, 1);
      addJustified(document,
          "5.\tОплата осуществляется после заключения настоящего договора в срок с    15.09.2025       по     10.10.2025.",
          11, false);
      addSpacer(document, 1);
      ...
      addJustified(document,
          "10.\tЗа неисполнение или ненадлежащее исполнение своих обязательств по настоящему договору стороны несут ответственность "
              + "в соответствии с законодательством Республики Беларусь.",
          11, false);
      addJustified(document,
          "11.\tПри нарушении сроков оплаты, предусмотренных пп. 4 и 5 настоящего договора, Слушатель выплачивает пеню в размере "
              + "0,1% от суммы просроченных платежей за каждый день просрочки.",
          11, false);
      addJustified(document,
          "12.\tСлушатель несет ответственность перед БНТУ за причинение вреда имуществу БНТУ в соответствии с законодательством Республики Беларусь.",
          11, false);
      addSpacer(document, 1);
      ...
      addJustified(document,
          "13.\tНастоящий договор составлен в 2 экземплярах, имеющих одинаковую юридическую силу, по одному для каждой из сторон.\n"
              ...
              + "18.\tВ целях оперативности и срочности решения вопросов настоящий договор и другие документы, касающиеся договора, "
              + "могут быть изготовлены и переданы посредством электронной и факсимильной связи, с последующим предоставлением "
              + "оригиналов в течение 10 рабочих дней. За достоверность документов и подписи стороны несут ответственность в "
              + "соответствии с действующим законодательством.\n"
      ...
      addLeft(document, "___________________________________________________", 11);
      addLeft(document, "Документ, удостоверяющий личность законного представителя ребёнка (вид, серия), номер, дата выдачи, наименование государственного органа, его выдавшего, идентификационный номер (при наличии):", 11);
      addLeft(document, "___________________________________________________", 11);
      addLeft(document, "___________________________________________________", 11);
      ...
      // dynamic student values appended at end for verification/use in generated contract
      addSpacer(document, 1);
      addLeft(document, "Данные из системы: " + fullName + ", дата рождения: " + birthDate + ", email: " + safe(person.getEmail()) + ", телефон: " + phone + ".", 10);
      addLeft(document, "Курс: " + safe(courseName) + ", семестр: " + safe(semesterName) + ", группа: " + safe(groupName) + ", преподаватель: " + safe(teacherName) + ".", 10);
      addLeft(document, "Базовая цена: " + basePrice.setScale(2, RoundingMode.HALF_UP).toPlainString() + ", скидка: " + discountPercent + "%.", 10);
