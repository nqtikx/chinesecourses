...
- Порт **8081** должен быть свободен (по умолчанию)
...
- Загрузка материалов своей группы
- Редактирование расписания своей группы
- Просмотр и скачивание договоров по своей группе
...
| Профиль | `/profile` | Профиль слушателя + текущий курс + история |
| Профиль класса | `/class-profile` | Группа, преподаватель, студенты, материалы |
| Таблица расписания | `/schedule-table` | Просмотр расписания и экспорт XLSX |
| Материалы | `/materials` | Загрузка и скачивание файлов по группе |
| Договоры | `/contracts` | Список договоров и генерация (admin) |
...
## Новые MVP-модули (расписание, материалы, договоры, профили)

### 1) Профиль слушателя
- `GET /api/profile/me` — текущий курс/группа + история completed.
- `PATCH /api/profile/me` — редактирование своего профиля (ФИО, дата рождения, email, телефон).
- В UI: страница `/profile`.

### 2) Профиль класса (группы)
- `GET /api/class-profiles/me` — профиль текущей группы слушателя.
- `GET /api/class-profiles/{groupId}` — профиль группы (с проверкой прав).
- В ответе: курс, семестр, преподаватель, расписание, материалы, список активных студентов.
- В UI: страница `/class-profile`.

### 3) Учебные материалы
- `POST /api/materials/upload?groupId={id}` (multipart) — загрузка файла (ADMIN/TEACHER).
- `GET /api/materials?groupId={id}` — список материалов группы.
- `GET /api/materials/{id}/download` — скачивание файла.
- `GET /api/materials/{id}/view` — inline-предпросмотр (PDF/изображения).
- Хранение: локально в `data/storage/materials` (или `/app/storage/materials` в Docker).

### 4) Таблица расписания + экспорт
- `GET /api/schedule-table/group/{groupId}` — табличное представление расписания.
- `GET /api/schedule-table/group/{groupId}/export` — экспорт XLSX.
- В UI: страница `/schedule-table`.

### 5) Договоры (Word + хранение)
- `POST /api/admin/contracts` — генерация договора в формате DOCX и сохранение в системе.
- `GET /api/admin/contracts` — список всех договоров (ADMIN).
- `GET /api/contracts/my` — список своих договоров.
- `GET /api/contracts?groupId={id}` — договоры группы (TEACHER/ADMIN).
- `GET /api/contracts/{id}/download` — скачивание договора.
- Хранение: `data/storage/contracts` (или `/app/storage/contracts` в Docker).

### Логика скидки
- Если у слушателя есть хотя бы один завершенный семестр (`COMPLETED`) — применяется скидка.
- Размер скидки настраивается: `APP_CONTRACT_REPEAT_DISCOUNT_PERCENT` (по умолчанию `10`).
- Базовая стоимость семестра настраивается: `APP_CONTRACT_BASE_PRICE` (по умолчанию `1200`).

---

## Работа через Postman (REST API)

Все API-эндпоинты находятся по адресу `http://localhost:8081/api/...`
