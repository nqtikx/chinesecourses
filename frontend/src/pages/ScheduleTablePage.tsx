...
const ScheduleTablePage = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await scheduleTableApi.get(groupId);
      setData(res.data);
    } catch {
  ...
  const exportXlsx = async () => {
    try {
      const res = await scheduleTableApi.exportXlsx(groupId);
    ...
    <div className="space-y-6">
      ...
      {data && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 overflow-x-auto">
          ...
          <p className="text-sm text-gray-500 mb-4">{data.courseName} | {data.semesterName} | {data.teacherName}</p>
